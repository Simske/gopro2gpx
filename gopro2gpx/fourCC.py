...
KARMAUNIT10Data = collections.namedtuple(
    "KARMAUNIT10Data", "A  Ah J degC V1 V2 V3 V4 s p1"
)
KARMAUNIT15Data = collections.namedtuple(
    "KARMAUNIT15Data", "A  Ah J degC V1 V2 V3 V4 s p1 e1 e2 e3 e4 p2"
)
GPSData = collections.namedtuple("GPSData", "lat lon alt speed speed3d")
KARMAGPSData = collections.namedtuple(
    "KARMAGPSData", "tstamp lat lon alt speed speed3d unk1 unk2 unk3 unk4"
)
...
class LabelSCAL(LabelBase):
    ...
    def build(klvdata):
        ...
        """
        if klvdata.repeat == 1:
            return LabelBase.build(klvdata)
...
class LabelGPSF(LabelBase):
    ...
    xlate = {0: "no lock (invalid GPS info)", 2: "lock 2D (ok)", 3: "lock 3D (ok)"}
...
class LabelUNIT(LabelBase):
    ...
    def build(klvdata):
        ...
        if len(data_tuple) == 5:
            data = UNITData._make(
                map(lambda x: x.decode("utf-8").strip("\0"), data_tuple)
            )
        else:
            data = None
        return data
    ...
    def build(klvdata):
        ...
        if not klvdata.rawdata:
            ...
            for r in range(klvdata.repeat):
                ...
                data_item = GPSData._make(
                    s.unpack_from(klvdata.rawdata[r * 4 * 5 : (r + 1) * 4 * 5])
                )
...
skip_labels = [
    ...
    # "FWVS", "KBAT", "ATTD", "GLPI", "VFRH", "BPOS", "ATTR",
    # "SIMU", "ESCS", "SCPR", "LNED", "CYTS", "CSEN"
]
...
def manage(klvdata):
    ...
    print(
        "Warning. fourCC Label '%s' not found. Please summit a issue to: %s"
        % (klvdata.fourCC, issue_url)
    )
    return False
